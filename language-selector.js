(function () {
  ...
  function installMessageNotifications(){
    ...
    const style=document.createElement('style');style.textContent='.se-dm-toast{position:fixed;top:max(12px,env(safe-area-inset-top));left:50%;transform:translate(-50%,-130%);width:min(440px,calc(100% - 24px));z-index:100000;background:#0b2b59;border:1px solid #f2c94c;border-radius:13px;padding:11px 13px;box-shadow:0 12px 32px #0009;color:#fff;transition:transform .25s;cursor:pointer}.se-dm-toast.show{transform:translate(-50%,0)}.se-dm-toast b{display:block;font-size:11px;color:#f2c94c}.se-dm-toast span{display:block;font-size:12px;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}';document.head.appendChild(style);
